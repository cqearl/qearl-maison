...
function draftPayload(row){try{return JSON.parse(row?.payload_json||"{}")}catch{return {}}}
async function getDraft(env,type,id){
  return await env.DB.prepare(
    "SELECT * FROM site_drafts WHERE entity_type=? AND entity_id=? ORDER BY id DESC LIMIT 1"
  ).bind(type,id).first();
}
async function upsertDraft(env,type,id,parentId,payload){
  const old=await getDraft(env,type,id);
  if(old){
    await env.DB.prepare(
      "UPDATE site_drafts SET parent_id=?,payload_json=?,updated_at=CURRENT_TIMESTAMP WHERE id=?"
    ).bind(parentId||null,JSON.stringify(payload),old.id).run();
    return old.id;
  }
  const r=await env.DB.prepare(
    "INSERT INTO site_drafts(entity_type,entity_id,parent_id,payload_json) VALUES(?,?,?,?) RETURNING id"
  ).bind(type,id||null,parentId||null,JSON.stringify(payload)).first();
  return r.id;
}
async function deleteDraft(env,type,id){
  await env.DB.prepare("DELETE FROM site_drafts WHERE entity_type=? AND entity_id=?").bind(type,id).run();
}
...
async function publicSite(request,env){
  ...
  const preview=!!(wantsPreview && await readSession(request,env));
  ...
  let pageOut={...p,is_visible:!!p.is_visible};
  if(preview){
    const pd=await getDraft(env,"page",p.id);
    if(pd)pageOut={...pageOut,...draftPayload(pd),id:p.id};
  }
  ...
  for(const sec of sr.results||[]){
    const ir=await env.DB.prepare("SELECT * FROM site_items WHERE section_id=? AND is_visible=1 ORDER BY sort_order,id").bind(sec.id).all();
    const items=[];
    for(const raw of ir.results||[]){
      let it={...raw,is_visible:!!raw.is_visible};
      if(preview){
        const d=await getDraft(env,"item",raw.id);
        if(d)it={...it,...draftPayload(d),id:raw.id,section_id:raw.section_id};
      }
      const mr=await env.DB.prepare("SELECT * FROM site_media WHERE item_id=? ORDER BY sort_order,id").bind(raw.id).all();
      items.push({...it,media:mr.results||[]});
    }

    if(preview){
      const nd=await env.DB.prepare(
        "SELECT * FROM site_drafts WHERE entity_type='new_item' AND parent_id=? ORDER BY id"
      ).bind(sec.id).all();
      for(const row of nd.results||[]){
        const d=draftPayload(row);
        items.push({
          id:"draft-"+row.id,section_id:sec.id,title:d.title||"",subtitle:d.subtitle||"",
          badge:d.badge||"",href:d.href||"",content_text:d.content||"",
          is_visible:d.isVisible!==false,sort_order:999999+row.id,media:d.media||[],_draft_id:row.id
        });
      }
    }
  ...
  return json({ok:true,preview,page:{...pageOut,sections}},200,{},request);
}

async function adminDrafts(request,env,p){
  const a=await readSession(request,env); if(!a)return json({error:"Unauthorized"},401,{},request);
  if(!sameOrigin(request))return json({error:"Invalid origin"},403,{},request);
  await ensureAdminSchema(env);
  const b=request.method==="GET"?{}:await readJson(request);

  if(p==="/api/admin/drafts/item"&&request.method==="POST"){
    const id=Number(b.itemId||0);
    if(id){
      const did=await upsertDraft(env,"item",id,b.sectionId||null,b.payload||{});
      return json({ok:true,draftId:did},200,{},request);
    }
    const r=await env.DB.prepare(
      "INSERT INTO site_drafts(entity_type,entity_id,parent_id,payload_json) VALUES('new_item',NULL,?,?) RETURNING id"
    ).bind(b.sectionId||null,JSON.stringify(b.payload||{})).first();
    return json({ok:true,draftId:r.id},200,{},request);
  }

  if(p==="/api/admin/drafts/page"&&request.method==="POST"){
    const id=Number(b.pageId||0);
    if(!id)return json({error:"Existing page required for draft preview"},400,{},request);
    const did=await upsertDraft(env,"page",id,b.parentId||null,b.payload||{});
    return json({ok:true,draftId:did},200,{},request);
  }

  if(p==="/api/admin/drafts/publish-item"&&request.method==="POST"){
    if(b.draftId){
      const row=await env.DB.prepare("SELECT * FROM site_drafts WHERE id=? AND entity_type='new_item'").bind(b.draftId).first();
      if(!row)return json({error:"Draft not found"},404,{},request);
      const d=draftPayload(row),sectionId=row.parent_id;
      const ord=await env.DB.prepare("SELECT COALESCE(MAX(sort_order),0)+1 n FROM site_items WHERE section_id=?").bind(sectionId).first();
      ...
      ).bind(sectionId,d.title||"Untitled item",d.subtitle||"",d.badge||"",d.href||"",d.content||"",d.isVisible===false?0:1,ord?.n||1).first();
      for(const m of d.media||[]){
        const mo=await env.DB.prepare("SELECT COALESCE(MAX(sort_order),0)+1 n FROM site_media WHERE item_id=?").bind(it.id).first();
        await env.DB.prepare("INSERT INTO site_media(item_id,url,public_id,width,height,media_type,alt,sort_order) VALUES(?,?,?,?,?,?,?,?)")
          .bind(it.id,m.url||"",m.publicId||"",m.width||0,m.height||0,"image",m.alt||"",mo?.n||1).run();
      }
      await env.DB.prepare("DELETE FROM site_drafts WHERE id=?").bind(row.id).run();
      return json({ok:true,itemId:it.id},200,{},request);
    }

    const id=Number(b.itemId||0);
    const row=await getDraft(env,"item",id);
    if(!row)return json({error:"No saved draft"},404,{},request);
    const d=draftPayload(row);
    await env.DB.prepare(
      "UPDATE site_items SET title=?,subtitle=?,badge=?,href=?,content_text=?,is_visible=?,updated_at=CURRENT_TIMESTAMP WHERE id=?"
    ).bind(d.title||"Untitled item",d.subtitle||"",d.badge||"",d.href||"",d.content||"",d.isVisible===false?0:1,id).run();
    await deleteDraft(env,"item",id);
    return json({ok:true,itemId:id},200,{},request);
  }

  if(p==="/api/admin/drafts/publish-page"&&request.method==="POST"){
    const id=Number(b.pageId||0);
    const row=await getDraft(env,"page",id);
    if(!row)return json({error:"No saved draft"},404,{},request);
    const d=draftPayload(row);
    await env.DB.prepare(
      "UPDATE site_pages SET title=?,slug=?,subtitle=?,parent_id=?,is_visible=?,updated_at=CURRENT_TIMESTAMP WHERE id=?"
    ).bind(d.title||"Untitled page",d.slug||("page-"+id),d.subtitle||"",d.parentId||null,d.isVisible===false?0:1,id).run();
    await deleteDraft(env,"page",id);
    return json({ok:true,pageId:id},200,{},request);
  }

  if(p.startsWith("/api/admin/drafts/")&&request.method==="DELETE"){
    const id=Number(p.split("/").pop());
    await env.DB.prepare("DELETE FROM site_drafts WHERE id=?").bind(id).run();
    return json({ok:true},200,{},request);
  }
...
export default {
  async fetch(request,env){
    ...
    try{
      ...
      if(p.startsWith("/api/admin/drafts/")) return await adminDrafts(request,env,p);
